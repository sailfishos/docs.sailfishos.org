...
```
Here, we first create a `Page` object which is simply a container for the contents of the page. With `allowedOrientations` we could restrict the allowed orientations of the page. Effectively the set of allowed orientations is an intersection of the allowed orientations in the `ApplicationWindow` and the `Page`.
...
Next, we add a pulldown menu with one menu item, labelled “Show Page 2″. `qsTr()` is a function which provides localised strings. Depending on the UI language of the device, the end user will see different texts.

We then attach an `onClicked` action to the `MenuItem` which will push the second page onto the top of the `pageStack` (provided by `ApplicationWindow`). Note that `PullDownMenu` and `PushUpMenu` must always be nested inside a `SilicaFlickable`, `SilicaListView`, or `SilicaGridView`.
